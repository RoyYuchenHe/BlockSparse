...
class Solver(ABSolver):
    ...
    def _GetExpansionScores(self):
        return [np.linalg.norm(self._proj(self.residual, block), ord=2) for block in self.A.blockList]

    def _GetShrinkingScores(self):
        subMat = self.A.GetSubMatrix(self.blockID)
        coef = np.linalg.lstsq(subMat, self.b, rcond=None)[0]
        return [np.linalg.norm(self.A.blockList[id] @ coef[n * self.A.groupSize:(n + 1) * self.A.groupSize], ord=2)
